...
func writeFile(pathRoot, repo string, text string) {
	...
	file, err := os.OpenFile(path+"/Dockerfile.lastCommit.json", os.O_CREATE|os.O_WRONLY, 0755)
	if err != nil {
		log.Fatalln(err)
	}
	defer file.Close()
...
func fetchLatestCommit(repos []string, token, outputPath, visitedPath string) {
	client := &http.Client{}
	for _, repo := range repos {
		log.Println(fmt.Sprintf("getting latest commit for %s", repo))
		url := fmt.Sprintf("https://api.github.com/repos/%s/commits?path=Dockerfile&page=1&per_page=1", repo)
  ...
  toVisit := filter(repos, visited)
	...
	for _, token := range tokens {
		...
		if remaining > 0 {
			...
			wg.Add(1)
			go func ()  {
				fetchLatestCommit(repos, token, outputPath, visitedPath)
				defer wg.Done()
			}()
		}
	}
