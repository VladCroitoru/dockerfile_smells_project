...
RUN yum install -y R firefox bwa samtools vlgothic-fonts ipa-mincho-fonts ipa-gothic-fonts ipa-pmincho-fonts ipa-pgothic-fonts net-tools zsh libevent ibus-kkc file bind-utils vcftools bedtools supervisor vlgothic-p-fonts libxml2 mock gcc rpm-build rpm-devel rpmlint make python bash coreutils diffutils patch rpmdevtools gtk3-devel ncurses-devel
...
ADD rpms/turbovnc-2.1.1.x86_64.rpm /
RUN yum install -y ./turbovnc-2.1.1.x86_64.rpm
