...
RUN echo "en_US.UTF-8 UTF-8" > /etc/locale.gen && \
    apt-get update && \
    apt-get install -y --no-install-recommends --no-install-suggests openjdk-8-jre-headless locales && \
    apt-get clean && \
    groupadd -g 10000 subsonic && \
    useradd -M subsonic -u 10000 -g 10000 && \
...
RUN dpkg -i /tmp/subsonic-$SUBSONIC_VERSION.deb && rm -f /tmp/*.deb && \
    ...
    mkdir /mnt/music && mkdir /mnt/podcasts
