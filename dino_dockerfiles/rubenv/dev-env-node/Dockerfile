...
RUN dnf install -y gnupg tar bzip2 xz git rubygem-sass make gcc gcc-c++ redis fontforge woff zopfli libpng-devel ImageMagick && dnf clean all
...
RUN npm -g install bower grunt-cli gulp \
  && npm cache clear
