...
RUN zypper --non-interactive remove autoconf automake binutils cpp cpp48 gcc gcc48 glibc-devel libasan0 libatomic1 libcloog-isl4 libgomp1 libisl10 libitm1 libltdl7 libmpc3 libmpfr4 libpcre16-0 libpcrecpp0 libpcreposix0 libstdc++-devel libstdc++48-devel libtool libtsan0 libxml2-devel libxml2-tools linux-glibc-devel m4 make ncurses-devel pcre-devel php5-devel php5-pear pkg-config readline-devel tack xz-devel zlib-devel
...
# Add files
ADD assets/conf/nginx.conf /etc/nginx/nginx.conf
ADD assets/conf/nginx-ssl.conf /etc/nginx/nginx-ssl.conf
ADD assets/conf/fastcgi.conf /etc/nginx/fastcgi.conf
ADD assets/conf/25-nginx /etc/init.simple/25-nginx
ADD assets/conf/25-php-fpm /etc/init.simple/25-php-fpm
ADD assets/conf/20-postfix /etc/init.simple/20-postfix
ADD assets/conf/10-boot-conf /etc/init.simple/10-boot-conf
ADD assets/app/35-phd /etc/init.simple/35-phd
ADD assets/app/40-aphlict /etc/init.simple/40-aphlict
ADD assets/conf/php-fpm.conf /etc/php5/fpm/php-fpm.conf
ADD assets/conf/php.ini /etc/php5/fpm/php.ini
...
ADD assets/app/sshd_config.phabricator /etc/phabricator-ssh/sshd_config.phabricator
ADD assets/app/45-phabricator-ssh /etc/init.simple/45-phabricator-ssh
ADD assets/app/phabricator-ssh-hook.sh /etc/phabricator-ssh/phabricator-ssh-hook.sh
