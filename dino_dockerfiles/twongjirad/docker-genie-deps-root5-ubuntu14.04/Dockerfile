...
RUN apt-get update && \
      apt-get install -y git dpkg-dev cmake g++ gcc binutils libx11-dev libxpm-dev libxft-dev libxext-dev libgsl0-dev gfortran libfftw3-dev libxml2-dev wget python subversion && \
    mkdir /usr/local/share/pythia && \
      cd /usr/local/share/pythia && \
      wget https://genie.hepforge.org/trac/export/7080/generator/trunk/src/scripts/build/ext/build_pythia6.sh && chmod a+x build_pythia6.sh && \
      ./build_pythia6.sh && \
    mkdir /usr/local/share/root && \
      ...
      make -j4 && \
      make install -j4 && \
      cd /usr/local/share && rm -r /usr/local/share/root && \
    cd /usr/local/share && \
