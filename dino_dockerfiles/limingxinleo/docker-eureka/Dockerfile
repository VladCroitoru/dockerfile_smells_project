...
RUN cd /tomcat/webapps &&\
  mkdir eureka &&\
  cd eureka &&\
  wget -q http://repo1.maven.org/maven2/com/netflix/eureka/eureka-server/1.8.7/eureka-server-1.8.7.war &&\
  jar xf eureka-server-1.8.7.war &&\
  rm eureka-server-1.8.7.war
