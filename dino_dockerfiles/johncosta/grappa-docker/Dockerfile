...
RUN echo "sudo sysctl -w kernel.shmmax=$((1<<30)) >/dev/null 2>/dev/null" >> /etc/profile
...
#
# recommended way to do this is to use the `setup` script to generate data volumes
VOLUME ["/grappa", "/build"]
WORKDIR /build
