...
LABEL PSVersion="v6.0.0-alpha.10"
...
# Add PowerNSX
ADD https://github.com/vmware/powernsx/archive/v2.zip /powershell
RUN mkdir ~/.local/share/powershell/Modules/PowerNSX
RUN unzip /powershell/v2.zip -d /powershell/
RUN ls  /powershell/
RUN cp /powershell/powernsx-2/PowerNSX.ps*1 ~/.local/share/powershell/Modules/PowerNSX/
