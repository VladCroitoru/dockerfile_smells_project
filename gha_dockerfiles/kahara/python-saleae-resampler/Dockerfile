...
# Base builder image #
######################
FROM python:3.8-alpine as builder_base
...
RUN --mount=type=ssh pip3 install wheel \
    ...
    && pip3 wheel --trusted-host 172.17.0.1 --find-links=http://172.17.0.1:3141 --wheel-dir=/tmp/wheelhouse -r /tmp/requirements.txt \
...
RUN --mount=type=ssh apk add --no-cache \
        ...
        tini \
    && chmod a+x /docker-entrypoint.sh \
    && pip3 install --trusted-host 172.17.0.1 --find-links=http://172.17.0.1:3141 --find-links=/tmp/wheelhouse/ /tmp/wheelhouse/saleae_resampler-*.whl \
    && rm -rf /tmp/wheelhouse/ \
    && saleae_resampler --defaultconfig > docker_config.toml \
    && true
