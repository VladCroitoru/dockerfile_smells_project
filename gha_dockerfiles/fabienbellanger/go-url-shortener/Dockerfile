...
RUN go build -a -installsuffix cgo -o go-url-shortener .

# Move to /dist directory as the place for resulting binary folder
WORKDIR /dist

# Copy binary from build to main folder
RUN cp /build/go-url-shortener /build/projects.json /build/favicon.png .
...
COPY --from=builder /dist/go-url-shortener /
